...
const styles = StyleSheet.create({
  ...
  header: { marginBottom: 20, borderBottomWidth: 2, borderBottomColor: '#1a3a5c', paddingBottom: 10 },
  title: { fontSize: 24, fontWeight: 'bold', color: '#1a3a5c', marginBottom: 5 },
  subtitle: { fontSize: 12, color: '#666' },
  section: { marginBottom: 15 },
  sectionTitle: { fontSize: 14, fontWeight: 'bold', color: '#1a3a5c', marginBottom: 8, borderBottomWidth: 1, borderBottomColor: '#eee', paddingBottom: 3 },
  row: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 },
  col: { flex: 1 },
  label: { fontSize: 10, color: '#666', marginBottom: 2 },
  value: { fontSize: 11, color: '#333' },
  totalsSection: { marginTop: 20, paddingTop: 10, borderTopWidth: 2, borderTopColor: '#1a3a5c' },
  totalRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 },
  totalLabel: { fontSize: 11, color: '#666' },
  totalValue: { fontSize: 11, fontWeight: 'bold' },
  grandTotal: { fontSize: 16, fontWeight: 'bold', color: '#1a3a5c', marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#ddd' },
...
export default function PDFDownload({ quotation, items, clientName, clientEmail, clientCompany, filename }: Props) {
  useEffect(() => {
    generatePDF();
  }, []);
  ...
  const generatePDF = async () => {
      ...
      <Document>
        <Page size="A4" style={styles.page}>
            ...
            <View key={idx} style={styles.section}>
              ...
              <View style={styles.row}>
                ...
                <View style={styles.col}>
                  ...
                  <Text style={styles.value}>
                    {Object.values(item.sizeBreakdown).reduce((a, b) => (a as number) + (b as number), 0) as number} unidades
                  </Text>
                </View>
              </View>
                ...
                <View style={{ marginTop: 8 }}>
                  ...
                  {item.customizations.map((c, ci) => (
                    <Text key={ci} style={styles.value}>
                      - {c.locationName}: {formatPrice(c.subtotal)}
                    </Text>
                  ))}
                </View>
              )}
          ...
          <Text style={styles.footer}>
            ...
            PrintyHub - ADMA FASHION | www.printyhub.com
          </Text>
        </Page>
      </Document>
    );
    ...
    link.href = url;
    link.download = filename || `cotizacion-${quotation.quotationNumber}.pdf`;
